...
def add_user_to_subscribers(number, user_id):
    ...
    if count_of_number == 1 and status == 1:
        ...
        if len(subscriptions) < subs_count:
            ...
            conn.execute('''UPDATE users SET balance = balance + 3 WHERE id = ?''', (user_id,))
            conn.commit()
...
def uban_user(id, decision):
    ...
    if decision == 1:
        count = cursor.execute('''SELECT COUNT(id) FROM black_list WHERE id = ?''', (id,)).fetchone()[0]
        if count == 1:
            cursor.execute('''DELETE FROM black_list WHERE id = ?''', (id,))
            conn.commit()
            conn.close()
        ...
        count = cursor.execute('''SELECT COUNT(id) FROM black_list WHERE id = ?''', (id,)).fetchone()[0]
        if count == 0:
            cursor.execute('''INSERT INTO black_list(id) VALUES(?)''', (id,))
            conn.commit()
            conn.close()
