...
main_menu.add('💠Кор кардан', '☑️Гирифтани подписчик')
main_menu.add('📂Профили ман', '⚙️Партнёрская программа')

admin_menu = InlineKeyboardMarkup()
statistics_bt = InlineKeyboardButton(text='📊 Статистикаи бот', callback_data='stat')
mail_bt = InlineKeyboardButton(text='✉️ Реклама', callback_data='mail')
give_uban_bt = InlineKeyboardButton(text='🚷 бан/разбан', callback_data='uban')
...
@dp.message_handler(lambda m: m.text == '📂Профили ман' and user_banned(m.from_user.id) == False)
async def profile_button_handle(m: types.Message):
    await m.reply(PROFILE(m), reply=False, parse_mode='Markdown')


@dp.message_handler(lambda m: m.text == '☑️Гирифтани подписчик' and user_banned(m.from_user.id) == False)
...
async def handle_get_sub_count(m: types.Message):
    if (m.content_type == 'text') and (m.text.isdigit() == True) and (
        ...
        confirmation_menu.add(InlineKeyboardButton(text='🚫 Отмена', callback_data='cancel_' + str(channel_stat[-1])),
                              InlineKeyboardButton(text='✅ Подтвердить',
                                                   callback_data='confirm_' + str(channel_stat[-1])))
        state = dp.current_state(user=m.from_user.id)
...
@dp.message_handler(lambda m: m.text == '💠Кор кардан' and user_banned(m.from_user.id) == False)
            ...
            if channel_to_subscribe not in black_list:
                ...
                if bot_status == "administrator":
                    ...
                    if status_of_user.status == 'left':
                        ...
                        subscribe_menu.add(InlineKeyboardButton(text='Ба канал аъзо шавед',
                                                                url='tg://resolve?domain=' + username.username))
                        subscribe_menu.add(InlineKeyboardButton(text='Проверка ', callback_data='sub_' + str(
...
@dp.message_handler(lambda m: m.text == '⚙️Партнёрская программа' and user_banned(m.from_user.id) == False)
